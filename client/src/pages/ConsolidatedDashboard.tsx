...
import { ChevronLeft, Download, Filter, Loader2, TrendingUp, Users, DollarSign, Package, AlertCircle, X } from 'lucide-react';
...
type DrillDownLevel = 'overview' | 'filial' | 'fornecedor' | 'comprador';

interface DrillDownState {
  level: DrillDownLevel;
  selectedItem?: string;
  breadcrumb: Array<{ label: string; level: DrillDownLevel; item?: string }>;
}
...
export default function ConsolidatedDashboard() {
  ...
  const [drillDown, setDrillDown] = useState<DrillDownState>({
    level: 'overview',
    breadcrumb: [{ label: 'Visão Geral', level: 'overview' }]
  });
  ...
  // Drill-down handlers
  const handleFilialClick = (filialName: string) => {
    setDrillDown({
      level: 'filial',
      selectedItem: filialName,
      breadcrumb: [
        { label: 'Visão Geral', level: 'overview' },
        { label: `Filial ${filialName}`, level: 'filial', item: filialName }
      ]
    });
  };

  const handleFornecedorClick = (fornecedorName: string) => {
    setDrillDown({
      level: 'fornecedor',
      selectedItem: fornecedorName,
      breadcrumb: [
        { label: 'Visão Geral', level: 'overview' },
        { label: fornecedorName, level: 'fornecedor', item: fornecedorName }
      ]
    });
  };

  const handleCompradorClick = (compradorName: string) => {
    setDrillDown({
      level: 'comprador',
      selectedItem: compradorName,
      breadcrumb: [
        { label: 'Visão Geral', level: 'overview' },
        { label: compradorName, level: 'comprador', item: compradorName }
      ]
    });
  };

  const handleBreadcrumbClick = (level: DrillDownLevel, item?: string) => {
    if (level === 'overview') {
      setDrillDown({
        level: 'overview',
        breadcrumb: [{ label: 'Visão Geral', level: 'overview' }]
      });
    } else if (level === 'filial' && item) {
      handleFilialClick(item);
    } else if (level === 'fornecedor' && item) {
      handleFornecedorClick(item);
    } else if (level === 'comprador' && item) {
      handleCompradorClick(item);
    }
  };

  // Dados filtrados por drill-down
  const getDetailedData = () => {
    if (drillDown.level === 'filial' && drillDown.selectedItem && data) {
      const filialData = data.aggregations.byFilial[drillDown.selectedItem];
      return {
        title: `Detalhes - Filial ${drillDown.selectedItem}`,
        stats: {
          'Nível de Serviço': `${(filialData.nivelServicoMedio * 100).toFixed(1)}%`,
          'Taxa de Ruptura': `${(filialData.rupturaPctMedio * 100).toFixed(1)}%`,
          'Estoque': `R$ ${(filialData.estoqueVenda / 1000000).toFixed(2)}M`,
          'Faturamento': `R$ ${(filialData.faturamento2026 / 1000000).toFixed(2)}M`
        }
      };
    } else if (drillDown.level === 'fornecedor' && drillDown.selectedItem && data) {
      const fornecedorData = data.aggregations.byFornecedor[drillDown.selectedItem];
      return {
        title: `Detalhes - ${drillDown.selectedItem}`,
        stats: {
          'Produtos': fornecedorData.totalProdutos,
          'Estoque': `R$ ${(fornecedorData.estoqueVenda / 1000000).toFixed(2)}M`,
          'Faturamento': `R$ ${(fornecedorData.faturamento2026 / 1000000).toFixed(2)}M`,
          'Ticket Médio': `R$ ${(fornecedorData.faturamento2026 / Math.max(fornecedorData.totalProdutos, 1)).toFixed(0)}`
        }
      };
    } else if (drillDown.level === 'comprador' && drillDown.selectedItem && data) {
      const compradorData = data.aggregations.byComprador[drillDown.selectedItem];
      return {
        title: `Detalhes - ${drillDown.selectedItem}`,
        stats: {
          'Produtos': compradorData.totalProdutos,
          'Estoque': `R$ ${(compradorData.estoqueVenda / 1000000).toFixed(2)}M`,
          'Faturamento': `R$ ${(compradorData.faturamento2026 / 1000000).toFixed(2)}M`,
          'Ticket Médio': `R$ ${(compradorData.faturamento2026 / Math.max(compradorData.totalProdutos, 1)).toFixed(0)}`
        }
      };
    }
    return null;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-2 text-sm">
        {drillDown.breadcrumb.map((item, idx) => (
          <div key={idx} className="flex items-center gap-2">
            <button
              onClick={() => handleBreadcrumbClick(item.level, item.item)}
              className="text-cyan-600 hover:text-cyan-700 font-medium transition-colors"
            >
              {item.label}
            </button>
            {idx < drillDown.breadcrumb.length - 1 && (
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-3xl font-bold text-foreground mb-2">
            {detailedData ? detailedData.title : 'Dashboard Consolidado'}
          </h2>
          <p className="text-muted-foreground">
            {drillDown.level === 'overview' 
              ...
              : 'Clique em qualquer gráfico para expandir detalhes'}
          </p>
        </div>
      ...
      {detailedData && (
        <div className="card-metric bg-gradient-to-r from-cyan-50 to-blue-50 border border-cyan-200">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(detailedData.stats).map(([key, value]) => (
              <div key={key}>
      ...
      {drillDown.level === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="card-metric cursor-pointer hover:shadow-lg transition-shadow" onClick={() => handleFilialClick(chartData.filiais[0]?.fullName)}>
            <h3 className="text-lg font-semibold mb-4 text-foreground">
              Faturamento 2026 por Filial (Clique para expandir)
            </h3>
          ...
          {/* Top 10 Fornecedores */}
          <div className="card-metric cursor-pointer hover:shadow-lg transition-shadow" onClick={() => handleFornecedorClick(chartData.fornecedores[0]?.fullName)}>
            <h3 className="text-lg font-semibold mb-4 text-foreground">
              Top 10 Fornecedores (Clique para expandir)
            </h3>
            <ResponsiveContainer width="100%" height={300}>
          ...
          <div className="card-metric cursor-pointer hover:shadow-lg transition-shadow" onClick={() => handleCompradorClick(chartData.compradores[0]?.name)}>
            <h3 className="text-lg font-semibold mb-4 text-foreground">
              Distribuição por Comprador (Clique para expandir)
            </h3>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
              ...
              <tbody>
                {chartData.filiais.map((filial, idx) => (
                  <tr 
                    ...
                    onClick={() => handleFilialClick(filial.fullName)}
                  >
